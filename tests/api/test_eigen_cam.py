import numpy as np
import pytest
from api.eigen_cam import compute_eigen_cam, upsample
...
def test_output_shape_matches_feature_map_dimensions():
    fm = make_feature_map(4, 7, 7)
    result = compute_eigen_cam(fm)
...
def test_upsample_to_larger_target():
    fm = make_feature_map(4, 7, 7, fill=1.0)
    heatmap = compute_eigen_cam(fm)
    result = upsample(heatmap, target_h=480, target_w=640)
